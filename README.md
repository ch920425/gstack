...
| `/setup-gbrain` | **GBrain Onboarding** — from zero to running gbrain in under 5 minutes. PGLite local, Supabase existing URL, or auto-provision a new Supabase project via Management API. MCP registration for Claude Code plus default Codex/OpenCode configs when present, and per-repo trust triad (read-write/read-only/deny). [Full guide](USING_GBRAIN_WITH_GSTACK.md). |
...
**Sidebar agent — your AI browser assistant.** Type natural language in the Chrome side panel and a child agent executes it. "Navigate to the settings page and screenshot it." "Fill out this form with test data." "Go through every item in this list and extract the prices." On Codex installs, gstack defaults to GPT-5.5 for primary work and GPT-5.4-mini for quick utility work; Claude routing remains available only when a Claude host or provider is explicitly selected. Each task gets up to 5 minutes. The sidebar agent runs in an isolated session, so it won't interfere with your main terminal agent. One-click cookie import right from the sidebar footer.
...
After init, the skill offers to register gbrain as an MCP server for Claude Code and verifies default Codex/OpenCode local MCP configs when present, so `gbrain search`, `gbrain put_page`, etc. show up as first-class typed tools — not bash shell-outs. For Supabase/Postgres brains, local MCP configs include `GBRAIN_SKIP_AUTO_MIGRATE_ON_CONNECT=1`; run production schema migrations explicitly with `gbrain init --migrate-only` after a verified backup.
