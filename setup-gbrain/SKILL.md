...
**Scope honesty:** This skill's primary MCP registration step (5a) uses
`claude mcp add` for Claude Code. Step 5b also verifies the default local
OpenAI Codex CLI and OpenCode MCP config files when they exist on this
machine, using the same local stdio command (`gbrain serve`). Other hosts
(Cursor, Windsurf, etc.) still get the gbrain CLI on PATH and can register
the same command manually after setup.
...
## Step 5b: Verify Codex CLI and OpenCode default MCP config

This is best-effort and local-host-only. Do not put database URLs or tokens in
these files; credentials stay in `~/.gbrain/config.json`.

For local stdio GBrain, the command is always:

```json
{ "command": "<absolute path to gbrain>", "args": ["serve"] }
```

Use environment overrides only for connection and migration behavior, not secrets:
...
For Supabase/Postgres brains, this env var keeps MCP startup read-only with
respect to schema changes. Run production migrations explicitly with
`gbrain init --migrate-only` after a verified database backup.

### Codex CLI (`$CODEX_HOME/config.toml`)

If the Codex CLI config exists (`$CODEX_HOME/config.toml`, defaulting to
`$HOME/.codex/config.toml`), verify it has:
...
If the block is missing, add it. If it exists with a stale `command`, update
only the gbrain block. Do not rewrite unrelated Codex MCP servers.

### OpenCode (`~/.config/opencode/opencode.json`)

If `~/.config/opencode/opencode.json` exists, verify it has:
...
```json
{
  "mcp": {
    "gbrain": {
      "type": "local",
      "command": ["/absolute/path/to/gbrain", "serve"],
      "environment": {
        "GBRAIN_SKIP_AUTO_MIGRATE_ON_CONNECT": "1",
        "GBRAIN_DISABLE_DIRECT_POOL": "1",
        "GBRAIN_PREPARE": "false"
...
Preserve every existing key in `opencode.json`; only upsert `mcp.gbrain`.
If either host is currently running, tell the user to restart that host before
expecting the new tools to appear.
